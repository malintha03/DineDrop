...
public class OrderController {
    ...
    /** Build the customer-facing subtotal (= base + commission per item). */
    private double customerSubtotal(List<Cart> items) {
        double s = 0;
        for (Cart c : items) {
            s += (c.getPrice() + settingsService.commissionFromBase(c.getPrice())) * c.getQuantity();
        }
        return SettingsService.round2(s);
    }

    private void addCheckoutBreakdown(Model model, List<Cart> items, String couponCode) {
        double subtotal = customerSubtotal(items);
        String restaurantId = items.isEmpty() ? "" : items.get(0).getRestaurantId();
        double discount = 0;
        String couponMsg = "";
        boolean couponOk = true;
        if (couponCode != null && !couponCode.isBlank()) {
            CouponService.CouponResult r = couponService.apply(couponCode, restaurantId, subtotal);
            couponOk = r.ok;
            couponMsg = r.message;
            if (r.ok) discount = r.discount;
            model.addAttribute("appliedCoupon", r.ok ? r.code : "");
        }
    ...
    public String place(@RequestParam String address, @RequestParam String city,
        ...
        double subtotal = customerSubtotal(items);
        CouponService.CouponResult coupon = couponService.apply(couponCode,
                items.get(0).getRestaurantId(), subtotal);
    ...
    public String myOrders(HttpSession session, Model model,
        ...
        // Fetch all orders
        java.util.List<Order> all = orderService.byCustomer(u.getId());

        // Reverse the list so newest orders are at the top (index 0)
        java.util.Collections.reverse(all);

        model.addAttribute("orders", all);
        model.addAttribute("activeOrders", all.stream().filter(Order::isActive).toList());
        model.addAttribute("completedOrders", all.stream().filter(Order::isCompleted).toList());

        // Build riderId -> User map so customer can see rider phone & vehicle plate
        java.util.Map<String, User> riders = new java.util.HashMap<>();
        for (Order o : all) {
            String rid = o.getRiderId();
            if (rid != null && !rid.isBlank() && !riders.containsKey(rid)) {
                User r = userRepo.findById(rid);
                if (r != null) riders.put(rid, r);
            }
        }
    ...
    public String cancelOrder(@PathVariable String id, HttpSession session,
        ...
        User u = requireCustomer(session);
        if (u == null) return "redirect:/login";
        boolean ok = orderService.cancelByCustomer(id, u.getId());
        if (ok) ra.addFlashAttribute("cancelled", id);
        else ra.addFlashAttribute("cancelError", "Order cannot be cancelled — it has already been picked or processed.");
